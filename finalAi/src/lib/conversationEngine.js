...
export const handleImageUpload = async (file) => {
  ...
  // Stage 1: Chatbot confirms image receipt and initializes AI pipeline
  store.addMessage({
    sender: 'ai',
    type: 'text',
    text: `Visual asset received: "${file.name}" (${(file.size / 1024).toFixed(1)} KB). Initializing RoadLens AI infrastructure analysis pipeline. Running validation checks, visibility checks, and lighting calibration... All checks verified. Surface scanning sequence commencing.`
  });
  ...
  try {
    ...
    // Stage 2: Damage Detection Conversational Output
    store.addMessage({
      sender: 'ai',
      type: 'text',
      text: `Surface anomaly scan complete. Structural damage identified. Classification: ${analysisResult.damageType}. Running severity quantification... Rating is evaluated at ${analysisResult.severityScore}/10. Risk classification marked as ${analysisResult.riskLevel}. AI confidence score: ${analysisResult.confidenceScore}%.`
    });
    await delay(600);
    ...
    if (gpsCoords) {
      // Stage 3: Geolocation Resolution Auto-Detect message
      store.addMessage({
        sender: 'ai',
        type: 'text',
        text: `Road GPS coordinates resolved from image metadata: ${gpsCoords.latitude.toFixed(6)}, ${gpsCoords.longitude.toFixed(6)}. Mapping incident coordinates to the municipal geo-database...`
      });
      await delay(1000);
    ...
    if (resolvedLocation) {
      // Stage 3: Confirm Geolocation
      store.setLocationData(resolvedLocation);
      store.addMessage({
        sender: 'ai',
        type: 'text',
        text: `Geolocation resolved successfully: ${resolvedLocation.road}, ${resolvedLocation.city}, ${resolvedLocation.state}. Mapped incident to regional municipal database node.`
      });
      await delay(800);

      // Resume flow with detected location
      await finishWorkflow(analysisResult, resolvedLocation, store, workflowMsgId, file, base64Data, gpsCoords);

    } else {
      // Fallback Flow: Metadata is missing or geocoding failed
      store.setAnalyzing(false); // Pause active progress to await input
      
      // Stage 3: Geolocation Resolution Manual Fallback message
      store.addMessage({
        sender: 'ai',
        type: 'text',
        text: "Road location coordinates could not be automatically resolved from image metadata. Opening interactive regional map helper. Please pin the incident location or input address details below."
      });
      ...
      store.setOnLocationResolvedCallback((manualLocation) => {
        ...
        setTimeout(async () => {
          await finishWorkflow(analysisResult, mappedLocation, store, workflowMsgId, file, base64Data, gpsCoords);
        }, 600);
      });
 ...
 */
async function finishWorkflow(analysisResult, location, store, timelineMsgId, file, base64Data, gpsCoords) {
  // Step 6: Authority Mapping
  store.setActiveWorkflowStep(6);
  
  // Stage 4: Authority Identification conversational output
  store.addMessage({
    sender: 'ai',
    type: 'text',
    text: `Regional jurisdiction verified. Incident assigned to: ${location.authority}. Establishing secure communication routing to the designated municipal dispatch department.`
  });
  await delay(900);
  ...
  // Stage 5: Report Compilation conversational output
  store.addMessage({
    sender: 'ai',
    type: 'text',
    text: `Compiling official RoadLens Infrastructure Intelligence Report. Assembling GPS georeferencing, severity metrics, AI annotations, recommended actions, and municipal routing metadata...`
  });
  await delay(900);
  ...
  const reportData = {
    ...
    // Added data for PDF report & UI
    base64Image: base64Data ? `data:${file?.type || 'image/jpeg'};base64,${base64Data}` : null,
    gpsCoordinates: gpsCoords ? `${gpsCoords.latitude.toFixed(6)}, ${gpsCoords.longitude.toFixed(6)}` : "11.016800, 76.955800 (Manual Resolution)",
    summary: analysisResult.summary || "Visual infrastructure damage detected.",
    roadName: location.road || 'Incident Street',
  ...
  const summaryText = `Road infrastructure analysis completed successfully. Pavement deterioration has been identified near ${location.city}. The report is compiled and ready for municipal API transmission.`;
