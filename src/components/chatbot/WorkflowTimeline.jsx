...
export default function WorkflowTimeline({ isCollapsed }) {
  ...
  return (
    <div className={`w-full max-w-md mx-auto rounded-[32px] bg-white border border-slate-150/80 shadow-md shadow-slate-100/40 p-6 md:p-8 flex flex-col items-center transition-all duration-500 ${isCollapsed ? 'opacity-50 scale-98 pointer-events-none' : ''}`}>
      
      {/* Title Header */}
      <div className="text-center mb-6">
        <h4 className="text-[26px] font-extrabold text-primaryText tracking-tight leading-tight mb-2.5">Intelligence Workflow</h4>
        <p className="text-[14px] text-slate-600 font-medium">Running automated validation and analysis sequence</p>
      </div>
